...
- **`/start`済みの場合**([Issue #19](https://github.com/imifu/discord_viatual_friend_api/issues/19)): 画像をキャプチャでき次第すぐにチャンネルへ投稿し、その後バックグラウンドでRealtime APIの会話コンテキストへ`input_image`として送信して、AIに応答を促します(`response.create`)。**画像投稿自体はAPIとの通信を待たないため、即座に表示されます。このときAIが音声で応答するため、通常の音声出力コストが発生します。** 画像自体のコストは`SCREEN_CAPTURE_DETAIL`(既定 `low`、固定約85トークン相当)で低く抑えていますが、`/cap`を実行するたびに1回分の音声応答が発生する点に注意してください。
...
- **モデルが既に発話中に`/cap`を実行した場合**: 画像追加(`conversation.item.create`)はそのまま送信され、応答要求(`response.create`)は現在の発話が終わるまで自動的に遅延されます(Realtime APIは同時に1つの応答しか受け付けないため)。**同じ発話の終了を待っている間**に`/cap`を複数回実行した場合は、発話が終わった時点で送信済みの画像すべてを踏まえた応答が1回だけ生成されます(画像ごとに個別の音声応答が発生するわけではありません)。ただし、既に次の応答(前の`/cap`分の遅延応答)が始まった**後**に届いた`/cap`は、その応答自体には合流できません(開始済みの応答へ後から画像を追加することはできないため)。その場合は次の応答終了を待つ新しい遅延としてまとめ直され、結果として音声応答が2回に分かれます。短時間に`/cap`を連打した場合、応答の区切りをまたぐと複数回の音声応答が発生しうる点にご注意ください。
- 画像追加(`conversation.item.create`)・応答要求(`response.create`)のいずれも、**API側の肯定応答(`conversation.item.created`/`response.created`)を実際に受信して初めて成功と判断します。** 送信時に自前生成した32文字のID(Realtime APIの`item.id`は最大32文字までのため、標準的な36文字のUUIDはそのままでは使えません)を`event_id`と(画像は`item.id`、応答は`response.metadata.requestId`として)併せて送り、対応する肯定応答イベントが届いたときだけ成功、対応する`error`イベントが届いたとき、または5秒以内にどちらも届かなかったときは失敗として扱います(「エラーが返ってこなければ成功」という消極的な確認ではなく、通信断・極端な遅延のようなケースも失敗として検知できます)。
- 応答要求はさらに、`response.created`(応答オブジェクトが作られたこと=`accepted`)と`response.done`の`status`(実際にどう終わったか=`completed`)を別々に確認します。`status=failed`(生成そのものが失敗)、および`status=incomplete`のうち理由が`content_filter`(安全フィルタによる打ち切り)の場合は「反応が途中で失敗しちゃったみたい」と訂正します。`status=incomplete`のうち理由が`max_output_tokens`(`SCREEN_REACTION_MAX_OUTPUT_TOKENS`の上限到達)の場合や`status=cancelled`(発話中にユーザーが割り込んだ場合など)は、実際に何らかの音声が再生された可能性が高いため失敗として扱いません。`accepted`自体が失敗した場合は「反応できなかった」の1件だけを通知し、`accepted`成功後に`completed`が失敗した場合の「途中で失敗しちゃった」と二重に投稿されることはありません。
- 画像投稿自体はこれらの確認を待たずに即座に表示され、**成功時は追加のメッセージを送りません(音声応答そのものが確認になります)。拒否・失敗・タイムアウト・接続切断を検知した場合のみ**、同じチャンネルへ追加メッセージ(「さっき送った画像、AIには届かなかったみたい…ごめんね」/「さっき送った画像、反応できなかったみたい…ごめんね」/「さっき送った画像への反応が途中で失敗しちゃったみたい…ごめんね」)で知らせます。
...
| 29 | 画面キャプチャのRealtime API送信(Issue #19) | `/start`後に`/cap`を実行する | 画像(640×360)が即座にチャンネルへ投稿され(「AIにも送ったよ」の文言)、少し遅れてAIが画面内容を踏まえた音声で応答する。応答は概ね1文・5〜8秒程度に収まる(長すぎる場合は`SCREEN_REACTION_MAX_OUTPUT_TOKENS`を調整)。`/start`前に`/cap`した場合は画像投稿のみで音声応答は起きないことも合わせて確認する |
| 30 | AI発話中の`/cap`(1回) | AIが発話している最中に`/cap`を1回実行する | 画像は即座に投稿される(発話中でも待たされない)。発話が終わった後、画像内容を踏まえた短い音声応答(1文程度)が1回発生する |
| 31 | AI発話中の`/cap`(複数回、同じ発話待ち中) | AIが発話している最中に`/cap`を2〜3回、いずれも**その発話が終わる前に**連続で実行する | 画像はそれぞれ即座に投稿される。発話が終わった後、複数の`response.create`が競合エラーになることなく、短い音声応答が1回だけ発生する(2回に分かれないことを特に確認) |
| 32 | AI発話中の`/cap`(応答の区切りをまたぐ場合) | AI発話中に`/cap`を実行し、その遅延応答が始まった**後**にもう一度`/cap`を実行する | 1回目の画像は1回目の音声応答に、2回目の画像は(1回目の応答が終わった後の)2回目の音声応答に、それぞれ反映される(合計2回の短い音声応答になることを確認。1回にまとまらないのは既知の仕様) |
| 33 | 通常の音声会話への影響がないこと | `/cap`実行後、`/cap`を使わずに通常通りDiscordで話しかける | 通常の音声応答が、`/cap`用の短文指示(1文・出力上限)の影響を受けず、いつも通りの長さ・性格プロンプトのまま応答する |
| 34 | 画像投稿の即時性 | `/start`後に`/cap`を実行し、画像がチャンネルに表示されるまでの体感時間を確認する | 画像はキャプチャ後すぐ(API確認を待たず)表示される。数秒待たされる場合は`appendImage`/`requestResponseWhenIdle`がブロッキングしていないか実装を確認する |
