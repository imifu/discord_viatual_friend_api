...
- 応答要求はさらに、`response.created`(応答オブジェクトが作られたこと)と`response.done`の`status`(実際にどう終わったか)を別々に確認します。`status=failed`(生成そのものが失敗)の場合のみ「反応が途中で失敗しちゃったみたい」と訂正します。`status=incomplete`(`SCREEN_REACTION_MAX_OUTPUT_TOKENS`の上限に達した場合など)や`status=cancelled`(発話中にユーザーが割り込んだ場合など)は、実際に何らかの音声が再生された可能性が高いため失敗として扱いません。
