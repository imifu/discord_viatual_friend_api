...
- **モデルが既に発話中に`/cap`を実行した場合**: 画像追加(`conversation.item.create`)はそのまま送信され、応答要求(`response.create`)は現在の発話が終わるまで自動的に遅延されます(Realtime APIは同時に1つの応答しか受け付けないため)。発話中に`/cap`を複数回実行した場合も、発話が終わった時点で送信済みの画像すべてを踏まえた応答が1回だけ生成されます(画像ごとに個別の音声応答が発生するわけではありません)。画像追加と応答要求は互いの確認を待たずに送信するため、発話終了のタイミングをまたいで複数回`/cap`を実行しても、常に1回の応答にまとまります。
...
| 31 | AI発話中の`/cap`(複数回) | AIが発話している最中に`/cap`を2〜3回連続で実行する。うち少なくとも1回は、直前の`/cap`の画像投稿から間を置かずに(1秒以内など)実行する | 画像はそれぞれ即座に投稿される。発話が終わった後、複数の`response.create`が競合エラーになることなく、短い音声応答が1回だけ発生する(2回に分かれないことを特に確認) |
| 32 | 通常の音声会話への影響がないこと | `/cap`実行後、`/cap`を使わずに通常通りDiscordで話しかける | 通常の音声応答が、`/cap`用の短文指示(1文・出力上限)の影響を受けず、いつも通りの長さ・性格プロンプトのまま応答する |
| 33 | 画像投稿の即時性 | `/start`後に`/cap`を実行し、画像がチャンネルに表示されるまでの体感時間を確認する | 画像はキャプチャ後すぐ(API確認を待たず)表示される。数秒待たされる場合は`appendImage`/`requestResponseWhenIdle`がブロッキングしていないか実装を確認する |
