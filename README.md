...
- 画像追加(`conversation.item.create`)・応答要求(`response.create`)のいずれも、**API側の肯定応答(`conversation.item.created`/`response.created`)を実際に受信して初めて成功と判断します。** 送信時に自前生成したIDを`event_id`と(画像は`item.id`、応答は`response.metadata.requestId`として)併せて送り、対応する肯定応答イベントが届いたときだけ成功、対応する`error`イベントが届いたとき、または5秒以内にどちらも届かなかったときは失敗として扱います(「エラーが返ってこなければ成功」という消極的な確認ではなく、通信断・極端な遅延のようなケースも失敗として検知できます)。画像投稿自体はこの確認を待たずに即座に表示され、**成功時は追加のメッセージを送りません(音声応答そのものが確認になります)。拒否・タイムアウト・接続切断を検知した場合のみ**、同じチャンネルへ追加メッセージ(「さっき送った画像、AIには届かなかったみたい…ごめんね」/「さっき送った画像、反応できなかったみたい…ごめんね」)で知らせます。
