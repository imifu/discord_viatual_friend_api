...
```
https://discord.com/api/oauth2/authorize?client_id=<CLIENT_ID>&scope=bot%20applications.commands&permissions=3148800
```

- `<CLIENT_ID>` は手順4で控えた Application ID に置き換えてください
- `scope=bot%20applications.commands` はBot本体とスラッシュコマンドの両方に必須です
- `permissions=3148800` は 表示(View Channels)・メッセージ送信(Send Messages)・VC接続(Connect)・VC発話(Speak) の最小権限です
...
- 本アプリはあくまで半二重方式(発話検知による簡易的な排他制御)であり、完全なエコーキャンセル(AEC)ではありません。両者が同時に発話し続けるケースは根本対応できません(20章の制限事項も参照)
