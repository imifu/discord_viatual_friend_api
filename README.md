...
- 画像追加(`conversation.item.create`)・応答要求(`response.create`)のいずれも、送信した`event_id`に対応するAPI側の`error`イベントを数秒以内に検知するとエラー扱いにします。画像投稿自体はこの確認を待たずに即座に表示されるため、**成功時は追加のメッセージを送りません(音声応答そのものが確認になります)。拒否や接続切断を検知した場合のみ**、同じチャンネルへ追加メッセージ(「さっき送った画像、AIには届かなかったみたい…ごめんね」/「さっき送った画像、反応できなかったみたい…ごめんね」)で知らせます。ただし、これは「エラーが返ってこなかった」ことを消極的な成功確認として扱う設計であり(`response.created`そのものの到着を待つわけではない)、エラーにすらならない極端な遅延のようなケースでは検知できません(既知の制約)。
