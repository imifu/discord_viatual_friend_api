...
> **既知の不整合(要ユーザー確認):** リポジトリのデフォルトブランチは `master` だが、
> 実装のコミット履歴(Phase2〜5、初回実装)はすべて `main` 側にあり、`master` は
> 空コミット1つのみの状態になっている。このCLAUDE.mdおよび今回のPRは実体のある `main` を
> 基点・マージ先として扱った。`master` と `main` の統合方針は別途ユーザー判断が必要。
